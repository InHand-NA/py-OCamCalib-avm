...
W,H=None,None
aa = "python src/pyocamcalib/script/opencv_fe_intr_test.py -input image -path ./test_images/inhandus_1/ -image "
...
parser.add_argument('-image', '--IMAGE_FILE', default='IMG', type=str, help='Input Image File Name Prefix (eg.: img_raw)')
parser.add_argument('-mode', '--SELECT_MODE', default='auto', type=str, help='Image Select Mode: auto/manual')
parser.add_argument('-fw','--FRAME_WIDTH', default=1080, type=int, help='Camera Frame Width')
parser.add_argument('-fh','--FRAME_HEIGHT', default=1920, type=int, help='Camera Frame Height')
parser.add_argument('-bw','--BORAD_WIDTH', default=8, type=int, help='Chess Board Width (corners number)')
parser.add_argument('-bh','--BORAD_HEIGHT', default=6, type=int, help='Chess Board Height (corners number)')
parser.add_argument('-size','--SQUARE_SIZE', default=22.8  , type=int, help='Chess Board Square Size (mm)')
...
class InCalibrator:
    ...
    def _get_cornersSB(self, img):
        ret, corners = cv2.findChessboardCornersSB(img, (args.BORAD_WIDTH, args.BORAD_HEIGHT),
                                                   flags=cv2.CALIB_CB_EXHAUSTIVE + cv2.CALIB_CB_ACCURACY)
        #if ret:
        return ret, corners
        #return self.get_corners(img)
...
class CalibMode():
    ...
    def runCalib(self, raw_frame, display_raw=True, display_undist=False):
        ...
        ok, result = calibrator(raw_frame)
    ...
    def imageAutoMode(self):
        ...
        for filename in filenames:
            ...
            ok, result = self.runCalib(raw_frame)
            print(f"ok: {ok}")
            if ok:
                count += 1
            key = cv2.waitKey(1)
        ...
        print(f"Detect corners: {count}/{len(filenames)}")
        return result
